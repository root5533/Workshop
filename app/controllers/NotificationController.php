...
class NotificationController extends Controller {
    ...
    public function getStoreNotifications() {
        if (isset($_POST["view"])) {
            $connect = $this->db_connect();
            if ($_POST["view"] != '') {
                $update_query = "UPDATE storecomments SET comment_status=1 WHERE comment_status=0";
                mysqli_query($connect, $update_query) or die(mysqli_error($dbc));
            }
            $query = "SELECT * FROM storecomments ORDER BY comment_id DESC LIMIT 10";
            $result = mysqli_query($connect, $query);
            $output = '';
            ...
            $query_1 = "SELECT * FROM storecomments WHERE comment_status=0";
            $result_1 = mysqli_query($connect, $query_1) or die(mysqli_error($connect));
            $count = mysqli_num_rows($result_1);
            $data = array(
                'notification' => $output,
